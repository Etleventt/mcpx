...
func restrictedSessionData(s remotesession.Session) map[string]any {
	return map[string]any{"session_id": s.ID, "workspace": s.WorkspaceName, "status": s.Status, "mode": "folders", "file_only": true, "allowed_tools": []string{"session", "session_read", "workspace_read", "source_read", "change"}, "notice": "仅允许指定目录的文件读取、搜索和单文件编辑；终端、外部代理、MCP扩展和旧活动记录不可用。"}
}
